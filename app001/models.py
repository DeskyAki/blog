...
class Article(models.Model):
    ...
    def __str__(self):
        """返回模型的字符串表示"""
        small_text = self.desc[:50]
        if small_text == self.desc:
            return self.desc
        else:
            return self.desc[:50] + "..."
...
class ArticleDetail(models.Model):
    ...
    article = models.OneToOneField(to="Article", to_field="nid", verbose_name="关联的文章标题", on_delete=models.CASCADE)
...
class Article2Tag(models.Model):
    ...
    def __str__(self):
        ...
        return self.article.title[:20] + "---" + self.tag.title
        # 我贼尼玛 老子死也没想出阿来，怎么拼接
